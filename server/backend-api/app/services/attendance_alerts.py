...
async def process_monthly_low_attendance_alerts():
    ...
    """
    logger.info("Starting monthly low attendance alert processing via automatic scheduler...")
    ...
    for teacher in teachers:
        ...
        for subject in subjects:
            ...
            for student_record in students:
                # schema: { student_id: ObjectId, attendance: { present: int, absent: int, ... } }
                ...
                if percentage < 75.0:
                    ...
                    # 4. Send Email
                    try:
                        await BrevoEmailService.send_low_attendance_warning(
                        ...
                        emails_sent_count += 1
                        logger.info(f"Sent alert to {student_email} for {subject_name} ({percentage:.1f}%)")
                    except Exception as e:
                        logger.error(f"Failed to send alert to {student_email}: {e}")

    logger.info(f"Completed low attendance alert processing. Total emails sent: {emails_sent_count}")
